<template lang="pug">
div
  form(ref='fileform').form__upload-file
    span.upload-text Drag or drop to upload image
  progress(max='100' :value.prop='uploadPercentage')
  .file-listing(v-for='(file, key) in files')
    img.preview(v-bind:ref="'preview'+parseInt( key )")
    |       {{ file.name }}
    .remove-container
      a.remove(v-on:click='removeFile( key )') Remove
  a.submit-button(v-on:click='submitFiles()' v-show='files.length > 0') Submit
  </template>
